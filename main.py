...
class Config:
    ...
    data["level"] = load_data["level"]
    data["choice_numbers"] = load_data["choice_numbers"]
...
class Process:
    ...
    def __init__(self) -> None:
        ...
        self.comment_num = 0
        # 动态评论真实OID
        self.comment_id_str: str = ""
        # 动态评论类型
        self.comment_type: int = -1

    def run(self):
        """ 主线程  """
        # 获取动态的真实评论oid与Type
        self.process_oid()
        if self.comment_id_str == "" or self.comment_type == -1:
            logger.error("获取真实评论oid与Type失败，请联系开发人员进行排障。")
            return None
        ...
        while True:
            ...
            if request_times < Config.data['times'] and response is None:
                ...
                continue
            elif request_times >= Config.data['times']:
                logger.error("请求动态评论重试超过%s次，终止程序" % str(request_times))
                break
    ...
    def process_oid(self) -> None:
        """ 获取动态comment_id_str与comment_type """
        # 单次请求间隔0.5秒
        time.sleep(0.5)
        # 开始请求
        url: str = 'https://api.bilibili.com/x/polymer/web-dynamic/v1/detail?id=%d' % (
            Config.data['oid']
        )
        # UA配置
        ...
        try:
            ...
            if response["code"] != 0:
                ...
                self.comment_id_str: str = response["data"]["item"]["basic"]["comment_id_str"]
                self.comment_type: int = response["data"]["item"]["basic"]["comment_type"]
        except Exception as e:
            logger.error(e)
            return None      
